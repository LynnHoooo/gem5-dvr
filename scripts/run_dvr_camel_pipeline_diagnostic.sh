...
innermost_switches="$(stat dvrDiscoveryInnermostSwitches)"
...
line "innermost stride switches" "$innermost_switches"
...
add_chunks="$(stat dvrVectorAddChunkIssues)"
line "FU requests/grants/stalls" "$fu_requests/$fu_grants/$fu_stalls"
line "add/ALU/shift/multiply chunks" "$add_chunks/$(stat dvrVectorALUChunkIssues)/$(stat dvrVectorShiftChunkIssues)/$(stat dvrVectorMultiplyChunkIssues)"
...
positive "no vector add-class chunk reached SimdAddOp" "$add_chunks"
