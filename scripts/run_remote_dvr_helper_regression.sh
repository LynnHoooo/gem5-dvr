...
printf 'DVR_HELPER_REGRESSION_PASSED run=%s branch_stats=%s nested_stats=%s variable_stats=%s branch_max_group_width=%s nested_flattened_lanes=%s variable_lane_batches=%s\n' \
    "$RUN_ID" "$branch_stats" "$nested_stats" "$variable_stats" \
    "$branch_width" "$nested_flat" "$variable_batches"
