...
require_equal reconvergences "$reconvergences" "$branches"
...
printf 'DVR_STAGE11_CONTROL_PASSED starts=%s completions=%s abandons=%s programs=%s divergent=%s reconvergences=%s relations=%s selected_paths=%s dependent=%s timeouts=%s recorder_overflows=%s stack_overflows=%s forced_timeouts=%s forced_generated=%s\n' \
    "$starts" "$completions" "$abandons" "$programs" "$branches" \
    "$reconvergences" "$relations" "$paths" "$dependent" "$timeouts" \
