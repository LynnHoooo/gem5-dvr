...
helper_fu_requests="$(read_stat "$stats" system.cpu.dvrHelperFURequests)"
helper_fu_grants="$(read_stat "$stats" system.cpu.dvrHelperFUGrants)"
helper_fu_stalls="$(read_stat "$stats" system.cpu.dvrHelperFUStalls)"
...
for pair in cycles:$cycles issued:$issued helper_fetch:$helper_fetch helper_decode:$helper_decode helper_compute:$helper_compute helper_fu_requests:$helper_fu_requests helper_fu_grants:$helper_fu_grants helper_fu_stalls:$helper_fu_stalls helper_alu_ops:$helper_alu_ops helper_shift_ops:$helper_shift_ops helper_lsu_ops:$helper_lsu_ops conflicts:$conflicts issue_conflicts:$issue_conflicts alu_conflicts:$alu_conflicts lsu_conflicts:$lsu_conflicts main_issue:$main_issue main_suppress:$main_suppress dvr_issued:$dvr_issued; do
  test -n "${pair#*:}"
done
if (( cycles <= 0 || issued <= 0 || helper_fetch <= 0 || helper_decode <= 0 ||
      helper_compute <= 0 || helper_fu_requests <= 0 || helper_fu_grants <= 0 ||
      helper_fu_grants > helper_fu_requests || helper_alu_ops <= 0 || helper_shift_ops <= 0 ||
...
printf 'DVR_STAGE15_RESOURCE_PASSED cycles=%s helper_fetch=%s helper_decode=%s helper_compute=%s fu_requests=%s fu_grants=%s fu_stalls=%s alu_ops=%s shift_ops=%s lsu_ops=%s helper_issue_cycles=%s conflicts=%s issue_conflicts=%s alu_conflicts=%s lsu_conflicts=%s main_issue=%s main_thread_suppressed=%s dvr_issued=%s\n' \
  "$cycles" "$helper_fetch" "$helper_decode" "$helper_compute" "$helper_fu_requests" "$helper_fu_grants" "$helper_fu_stalls" "$helper_alu_ops" "$helper_shift_ops" "$helper_lsu_ops" "$issued" "$conflicts" "$issue_conflicts" "$alu_conflicts" "$lsu_conflicts" "$main_issue" "$main_suppress" "$dvr_issued"
