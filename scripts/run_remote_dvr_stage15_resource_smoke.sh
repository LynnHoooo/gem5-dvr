...
for pair in cycles:$cycles issued:$issued helper_fetch:$helper_fetch helper_decode:$helper_decode conflicts:$conflicts issue_conflicts:$issue_conflicts alu_conflicts:$alu_conflicts lsu_conflicts:$lsu_conflicts main_issue:$main_issue main_suppress:$main_suppress dvr_issued:$dvr_issued; do
  test -n "${pair#*:}"
done
if (( cycles <= 0 || issued <= 0 || helper_fetch <= 0 || helper_decode <= 0 || dvr_issued <= 0 )); then
...
printf 'DVR_STAGE15_RESOURCE_PASSED cycles=%s helper_fetch=%s helper_decode=%s helper_issue_cycles=%s conflicts=%s issue_conflicts=%s alu_conflicts=%s lsu_conflicts=%s main_issue=%s main_thread_suppressed=%s dvr_issued=%s\n' \
  "$cycles" "$helper_fetch" "$helper_decode" "$issued" "$conflicts" "$issue_conflicts" "$alu_conflicts" \
