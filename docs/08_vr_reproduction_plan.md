...
以向量化压榨 MLP"。两者共享大量底层机制，本分支将**复用** DVR 已通过的 Stage 1–13 实现
（见 §5），只补 VR 特有的部分。**本分支不复制、不改写 `main` 的 DVR 逻辑。**
...
论文 §VI 的量化主张，作为本分支"机制生效"的验收靶：

| 指标 | 论文数值 | 本分支验收口径 |
|---|---|---|
| 整体性能 | VR 几何平均 **1.79×** vs OoO 基线；vs PRE 1.49× | 微基准/ROI 上 speedup > 1，并展示机制收益拆分（非必须复刻 1.79） |
...
`main` 分支 DVR 已通过 Stage 1–13（`docs/02_reproduction_status.md`）。与本论文重叠的机制：

| 机制 | 本论文（VR） | 现有 DVR 实现 | 复用方式 |
|---|---|---|---|
| stride 检测 | RPT + confidence + terminator | 32-entry RPT / stride candidate（Stage 1） | 直接复用，补 terminator 字段 |
| taint 跟踪 | TV：vectorize/invalid 2 bit | 32-register VTT/FLR（Stage 4） | 直接复用 |
| 间接链/多级 dependent | gather 链 + terminator | 两级 dependent prefetch（Stage 7–8） | 复用 |
| 寄存器簿记 | VRAT + RDQ | VRAT/VIR + 8-uop recorder（Stage 10） | 复用 VRAT，补 RDQ |
...
**VR 特有的、需要新写的部分：**
1. **向量化器**：标量指令 → 512-bit/RVV 向量指令 + gather 生成（DVR 是时序注入，
   本论文是真正的向量指令形态）。
2. **进入条件**：ROB 满 / IQ 80% 触发（DVR 是"未停时主动预取"）。
3. **终止条件**：再遇同 striding load / terminator / 全 lane invalid / 200 指令超时
   （DVR 的 Discovery 结束条件是循环界）。
4. **Unrolling (U) + Pipelining (P)**：多轮向量化 + 软件流水化重叠（全新结构）。
...
沿用仓库 `Stage N` 惯例，每个 Stage 有独立验收脚本和硬断言。**建议实现顺序**（每阶段可
单独提交）：

| Stage | 内容 | 验收点（证据类型） | 依赖 |
|---|---|---|---|
| **0** | 分支基建：RVV/向量指令支持确认，OoO 配置与 RPT 复用基线 | gem5 可跑含向量指令的 smoke；基线 config 正确 | — |
| **1** | VR 进入条件：ROB 满 / IQ 80% 检测 + PC/RAT checkpoint/恢复 | 触发/恢复事件计数，状态还原断言 | Stage 0 |
| **2** | 向量化器 v1：striding load → 向量 load/gather，算术依赖指令 → 向量指令 | 向量指令生成数、lane 数正确 | Stage 1 |
| **3** | 污点向量 TV + 传播 + invalid 丢弃 | tainted/invalid 事件计数，与 DVR Stage 4 对照 | Stage 2 |
| **4** | VRAT + RDQ：向量重命名 + 按序寄存器回收 | 重命名守恒、RDQ 释放正确、无寄存器泄漏断言 | Stage 3 |
| **5** | 终止条件四项 + U/P 轮次控制 | 各类终止计数、轮次守恒 | Stage 4 |
| **6** | 控制流 mask（1st-lane 方向 + 非法 lane mask） | divergent/masked 计数、掩码持久性 | Stage 5 |
| **7** | Unrolling (U) + Pipelining (P) 完整实现 | 64 标量等价 gather 发出、MLP 提升 | Stage 6 |
| **8** | 微基准：Camel/HJ2/Kangaroo/RandAcc 移植 + 编译 | 可跑通、ROI 提取 | Stage 7 |
| **9** | 指标采集：speedup / MLP / coverage / accuracy / timeliness | 与 §4 表格口径对齐 | Stage 8 |
| **10** | 敏感性：U/P、MSHR、LLC 扫描 + 完整回归脚本 | 趋势一致 + 全回归硬断言通过 | Stage 9 |

每个 Stage 完成 = 写清证据（事件计数、config 检查、与 DVR/论文对照）、脚本硬断言通过、
在本文档"进度台账"登记。**不以绝对 speedup 数字作为完成门槛**（本平台无法逐周期对齐
Sniper），以"机制行为与论文一致"为准。
